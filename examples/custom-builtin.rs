//! Example of implementing a custom builtin command for a kodegen_bash_shell based shell.
//!
...
//! ```bash
//! cargo run --example custom-builtin
//! ```
...
use kodegen_bash_shell::{builtins, BuiltinError, Error, ExecutionExitCode, ExecutionResult};
...
impl BuiltinError for GreetError {}
...
impl From<&GreetError> for ExecutionExitCode {
...
impl builtins::Command for GreetCommand {
    ...
    // the default-provided `kodegen_bash_shell::Error` type.
    type Error = GreetError;

    async fn execute(
        &self,
        context: kodegen_bash_shell::ExecutionContext<'_>,
...
async fn run_example() -> Result<()> {
    ...
    let mut shell = kodegen_bash_shell::Shell::builder()
        .builtin("greet", builtins::builtin::<GreetCommand>())
        .build()
        .await?;

    // Demonstrate basic usage.
    let result = shell
        .exec("greet -n 4", &shell.default_exec_params())
        .await?;
