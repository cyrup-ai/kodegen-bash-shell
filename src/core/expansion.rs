...
use crate::parser::word::ParameterTransformOp;
use crate::parser::word::SubstringMatchKind;
use itertools::Itertools;

use super::ExecutionParameters;
use super::arithmetic;
use super::arithmetic::ExpandAndEvaluate;
use super::braceexpansion;
use super::commands;
use super::env;
use super::error;
use super::escape;
...
use super::sys;
use super::trace_categories;
use super::variables::ShellValueUnsetType;
use super::variables::ShellVariable;
use super::variables::{self, ShellValue};
...
impl From<ExpansionPiece> for super::regex::RegexPiece {
    fn from(piece: ExpansionPiece) -> Self {
        match piece {
            ExpansionPiece::Unsplittable(s) => Self::Literal(s),
            ExpansionPiece::Splittable(s) => Self::Pattern(s),
...
pub(crate) async fn basic_expand_regex(
    ...
    params: &ExecutionParameters,
    word: &ast::Word,
) -> Result<super::regex::Regex, error::Error> {
    let mut expander = WordExpander::new(shell, params);
...
impl<'a> WordExpander<'a> {
        ...
        let regex_pieces: Vec<_> = expansion
            ...
            .map(|field| {
                field
                    .0
                    .into_iter()
                    .map(super::regex::RegexPiece::from)
                    .collect::<Vec<_>>()
            })
            .intersperse(vec![super::regex::RegexPiece::Literal(String::from(" "))])
            .flatten()
            .collect();

        Ok(super::regex::Regex::from(regex_pieces)
    ...
    async fn basic_expand(&mut self, word: &str) -> Result<Expansion, error::Error> {
        ...
        for piece in crate::parser::word::parse(brace_expanded.as_str(), &self.parser_options)? {
    ...
    async fn expand_word_piece(
        ...
        let expansion: Expansion = match word_piece {
            ...
            crate::parser::word::WordPiece::TildePrefix(prefix) => Expansion::from(
    ...
    async fn expand_parameter_without_indirect(
        ...
        match parameter {
            crate::parser::word::Parameter::Positional(p) => {
                if *p == 0 {
                    Ok(self
                        .expand_special_parameter(&crate::parser::word::SpecialParameter::ShellName))
