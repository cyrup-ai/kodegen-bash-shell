...
impl<'a> StyledInputLine<'a> {
    fn new(shell: &'a crate::core::Shell, input_line: &'a str, cursor: usize) -> Self {
        Self {
            shell,
            cursor,
    ...
    fn style_and_append_word_piece(
        ...
        match word_piece.piece {
            ...
            | crate::parser::word::WordPiece::TildePrefix(_) => {
                self.append_style(
                    styles::parameter(),
                    global_offset + word_piece.start_index,
                    global_offset + word_piece.end_index,
    ...
    fn get_style_for_word(
        &self,
        w: &str,
        token_location: &crate::parser::TokenLocation,
