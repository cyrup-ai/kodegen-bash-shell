...
use super::args::{CommandLineArgs, InputBackend};
use super::brushctl;
use super::error_formatter;
use super::events;
use super::productinfo;
use super::shell_factory;
...
impl CommandLineArgs {
    ...
    fn try_parse_from(itr: impl IntoIterator<Item = String>) -> Result<Self, clap::Error> {
        let (mut this, script_args) = crate::core::builtins::try_parse_known::<Self>(itr)?;
...
async fn run_in_shell(
    shell: &mut impl crate::interactive::InteractiveShell,
    args: CommandLineArgs,
) -> Result<u8, crate::interactive::ShellError> {
...
async fn instantiate_shell(
    ...
    cli_args: Vec<String>,
    factory: impl shell_factory::ShellFactory + Send + 'static,
) -> Result<impl crate::interactive::InteractiveShell + 'static, crate::interactive::ShellError> {
    ...
    let builtins = crate::builtins::default_builtins(if args.sh_mode {
        crate::builtins::BuiltinSet::ShMode
    } else {
    ...
    let options = crate::interactive::Options {
        shell: crate::core::CreateOptions {
...
fn new_error_formatter(
    args: &CommandLineArgs,
) -> Arc<Mutex<dyn crate::core::error::ErrorFormatter>> {
